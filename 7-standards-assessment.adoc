...
of Manhattan using 3D Tiles. It can be rendered in the Cesium globe (See Fig 1). A prototype implemetation is available at http://81.169.187.7:8081/3DPS_App/.
...
The current OpenAPI implementation supports two functionalities **Data Layer** and **3D Scene** (See Fig. 2). 
...
Since this call does not take any input you will see the output as shown in Fig 3. In addition, it also shows the curl request with all the necessary query parameters.

The **3D Scene** function on the other hand takes three input. The first is the path parameter(_layer_) and the remaining two are query parameters(_format_,_bbox_) see Fig 4
...
This _tilesetLink_ can be rendered in the Cesium globe. To show the feasibility of our approach we have developed an intergrated  prototype see Fig 5. Please visit the following http://81.169.187.7:8081/3DPS_App/[ http://81.169.187.7:8081/3DPS_App/]. 
