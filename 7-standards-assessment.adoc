...
3D CityGML model of New York described http://docs.opengeospatial.org/per/17-046.html[here]. The implementations is based on openAPI and the Web API can be tested http://steinbeis-3dps.eu:8080/tb14/API/index.html[http://steinbeis-3dps.eu:8080/tb14/API/index.html].  

Alternatively, to use the Web API call in your application, make the following request by defining the correct `bbox` coordinates in the request:
...
of Manhattan using 3D Tiles. It can be rendered in the Cesium globe (See <<img-App>>.). A prototype implemetation is available at http://81.169.187.7:8081/3DPS_App/.


[#img-App,reftext='{figure-caption} {counter:figure-num}']
.Cesium Globe 3D Tiles Manhattan 
image::images/3dps6.png[align="center"]

The current OpenAPI implementation supports two functionalities **Data Layer** and **3D Scene** (See <<img-App2>>.). 

[#img-App2,reftext='{figure-caption} {counter:figure-num}']
.OpenAPI definition 
image::images/3dps1.png[align="center"]

The **data Layer** function is used to retrieve the name of the available data layers. The current implementation supports one layer only:  _“3D_CityModel_manhattan”_ . However, in case the service supports more then one data layer it will show a list of supported data layers.

----
{“datalayer ”: “3D_CityModel_manhattan,3D_CityModel_geneva,3D_CityModel_stuttgart"}
----

NOTE: A simplified approach in taken in the prototype.

To test the Web API, first click on the _Try it out_  button and then the _Execute_ button.

[#img-App3,reftext='{figure-caption} {counter:figure-num}']
.OpenAPI Data Layer
image::images/3dps2.png[align="center"]

Since this call does not take any input you will see the output as shown in <<img-App3>>. In addition, it also shows the curl request with all the necessary query parameters.

The **3D Scene** function on the other hand takes three input. The first is the path parameter(_layer_) and the remaining two are query parameters(_format_,_bbox_) see <<img-App4>>

[#img-App4,reftext='{figure-caption} {counter:figure-num}']
.OpenAPI 3D Scene
image::images/3dps3.png[align="center"]

A Successful execution will provide the a tileset link (see example below)

----
tilesetLink :http://tb13.igd.fraunhofer.de:8083/3dps/datahoster/fc38b549-4989-4703-bb27-f5b4fd33fd90/manhattan/tileset.json
----

This _tilesetLink_ can be rendered in the Cesium globe. To show the feasibility of our approach we have developed an intergrated  prototype see <<img-App5>>. Please visit the following http://81.169.187.7:8081/3DPS_App/[ http://81.169.187.7:8081/3DPS_App/]. 

[#img-App5,reftext='{figure-caption} {counter:figure-num}']
.3DPS APP
image::images/3dps5.png[align="center"]

To educate yourself and get use the platform please visit the you tube link https://www.youtube.com/watch?v=6EwlwAhtK7Q[https://www.youtube.com/watch?v=6EwlwAhtK7Q].
