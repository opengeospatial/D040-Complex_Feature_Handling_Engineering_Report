...
3D CityGML model of New York described http://docs.opengeospatial.org/per/17-046.html[here]. The implementations is based on OpenAPI and the Web API can be tested at http://steinbeis-3dps.eu:8080/tb14/API/index.html[http://steinbeis-3dps.eu:8080/tb14/API/index.html].
...
The current OpenAPI implementation supports two resources **Data Layer** and **3D Scene** (See <<img-App2>>.).

[#img-App2,reftext='{figure-caption} {counter:figure-num}']
.OpenAPI definition
image::images/3dps1.png[align="center"]

The **data Layer** resource is used to retrieve the name of the available data layers. The current implementation supports one layer only:  _“3D_CityModel_manhattan”_ . However, in case the service supports more then one data layer it will show a list of supported data layers.
...
This _tilesetLink_ can be rendered in the Cesium globe. To show the feasibility of our approach we have developed an integrated prototype see <<img-App5>>. Please visit the following http://81.169.187.7:8081/3DPS_App/[ http://81.169.187.7:8081/3DPS_App/].
