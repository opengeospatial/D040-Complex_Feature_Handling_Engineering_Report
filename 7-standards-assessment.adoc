...
used to retrieve the name of the available data layers. The implementation supports one layer only:  “3D_CityModel_manhattan” . However in case the service supports more then one data layer will be a list of supported data layers (eg. {“datalayer ”: “3D_CityModel_manhattan,3D_CityModel_geneva,3D_CityModel_stuttgart"}. The REST API call can be tested by first clicking on the _Try it out_  button and then _execute_ button. 
...
You can also visit the you tube link https://www.youtube.com/watch?v=G_gpPgISNww [https://www.youtube.com/watch?v=G_gpPgISNww] to educate yourself how to use the platform.
