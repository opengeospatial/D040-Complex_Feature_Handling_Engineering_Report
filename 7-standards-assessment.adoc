...
As shown in Fig xxx The current implementation supports two functionalities Data Layer and 3D Scene.
The data Layer function is used to retrieve the name of the available data layers. The implementation supports one layer only:  “3D_CityModel_manhattan” . However in case the service supports more then one data layer will be a list of supported data layers (eg. {“datalayer ”:: “3D_CityModel_manhattan,3D_CityModel_geneva,3D_CityModel_stuttgart"}. the REST API call can be tested by first clicking on the Try it out  button and then execute button. Since the call does not take any input you will see the output as shown in Fig xxx. In addition, it also shows the curl request with all the necessary query parameters.

image::images/3dps1.png


As shown in Fig 1 The current implementation supports two functionalities Data Layer and 3D Scene
The data Layer function is used to retrieve the name of the data layer. In our implementation we only support “3D_CityModel_manhattan” . However in case the service supports more then one data layer then the you will get a list of supported data layers (eg. {“datalayer ”:: “3D_CityModel_manhattan,3D_CityModel_geneva,3D_CityModel_stuttgart"}. You can test the REST API call by first clicking on the Try it out  button and then execute button. Since the call does not take any input you will see the output as shown in Fig 2.



In addition, it also shows the curl request with all the necessary query parameters.

The 3D Scene function takes three input. The first is the path parameter(layer) and the remaining two are query parameters(format,bbox) see Fig:3


Successful execution will provide the a tilesetlink (see example below)
  {
{"tilesetLink" ::"http://tb13.igd.fraunhofer.de:8083/3dps/datahoster/82b6c8b4-7be6-408b-969b-733ecdeb4006/manhattan/tileset.json"}


This tilesetLink can be rendered in the Cesium globe. To show the feasibility of our approach we have developed a prototype see Fig 4. Please visit the following link to try it out. Please visit the you tube link to educate yourself how to use the platform.
